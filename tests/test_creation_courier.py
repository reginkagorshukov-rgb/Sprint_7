...
class TestCreateCourier:
    ...
    def test_create_courier_success_with_generator(self):
        ...
        response = requests.post( f'{BASE_URL}/api/v1/courier/login', data={"login": login, "password": password})
        assert response.status_code == 200
        
        auth_response = requests.post(f'{BASE_URL}/api/v1/courier/login',data={"login": login, "password": password})
    ...
    def test_request_returns_correct_code(self):
    ...
    @pytest.mark.parametrize('missing_field', ['login', 'password',])
    def test_create_courier_missing_fields_error(self, missing_field):
        login, password, first_name = register_new_courier_and_return_login_password()
